...
async def deepfry(img: Image, *, token: str = None, api_url: str = None, session: aiohttp.ClientSession = None,
    ...
    if kind not in DeepfryTypes:
        raise ValueError(
            f'Unknown deepfry type "{kind}", expected a value from deeppyer.DeepfryTypes')

    if token and api_url:
        # WHY THE FUCK IS THIS SO LONG
        req_url = f'{api_url}/detect?returnFaceId=false&returnFaceLandmarks=true'
        headers = {
    ...
    if token and face_data:
        ...
        eye_left_width = math.ceil(
            landmarks['eyeLeftInner']['x'] -
            landmarks['eyeLeftOuter']['x'])
        eye_left_height = math.ceil(
            landmarks['eyeLeftBottom']['y'] -
        ...
        eye_left_corner = tuple(
            math.floor(
                ...
                flare_left_size /
                2.5 +
                5) for x in eye_left_corner)

        eye_right_width = math.ceil(
            landmarks['eyeRightOuter']['x'] -
            landmarks['eyeRightInner']['x'])
        eye_right_height = math.ceil(
            landmarks['eyeRightBottom']['y'] -
